...
reset:
	...
	ser temp
	out ddrb,temp               ;B eksodos 
	
	rcall usart_init            ;Arxikopoihsh uart
    rcall ADC_init              ;Arxikopoihsh ADC
	rcall lcd_init				;Arxikopoihsh lcd
    clr data                    ;Arxikopoihsh kyriou metrhth
    sei                         ;Energopoioume tis diakopes

main:
	sbi ADCSRA,ADSC             ;Ksekiname mia metrhsh tashs
    out PORTB,data              ;Eksodos sta led B o metrhths
    inc data                    ;Ayksanoume to metrhth
    ldi r24,low(100)    
    ldi r25,high(100)
    rcall wait_msec             ;Perimenoume 100 msec
    rjmp main
    


adc_intr:                       ;Otan oloklhrw8ei h metrhsh, ginetai diakoph
	push counter                ;H timh ADC(10-bit) exei 2 bit ston ADCH kai 8 bit ston ADCL
	push r25                    ;Prokeimenou na paroume kai to prwto klasmatiko pshfio,
    push r24                    ;8a pollaplasiasoume epipleon me to 10 kai 8a pai3oume me ta 2 teleytaia dec pshfia
	push temp
	push temp2

	in temp,ADCL                ;ADCL ston temp
	in temp2,ADCH               ;ADCH ston temp2
	andi temp2,0x03             ;Maska gia na krathsoume ta 2 (egkyra) lsb, ta ypoloipa isws einai 8orybos
	ldi counter, 50

	clr temp3                   
	clr temp4

	add_50_times:               ;Pollaplasiazoume epi 50, pros8etontas 50 fores thn timh
		add temp3,temp
		adc temp4,temp2
		dec counter
		brne add_50_times

	mov temp, temp3             ;To low byte tou (ADC*50) ston temp
	mov temp2, temp4            ;To high byte tou (ADC*50) ston temp2
	ldi counter,10
	shift_10_times:             ;Diaroume me to 1024 kanontas 10 deksia shift
		lsr temp                ;Deksi shift to low
		sbrc temp2,0            ;Eksetazoume to lsb tou high
		ori temp,0x80           ;Ann einai 1, tote bazoume 1 sto neo msb tou low
		lsr temp2               ;Deksi shift to high
		dec counter
		brne shift_10_times
	
	ldi r24,0x80				;Deixnoume sthn prwth 8esh ths lcd
	rcall lcd_command

	mov r24,temp
	rcall convert_bin_to_dec_unsigned   ;Briskoume poses dekades kai monades exoume
	rcall get_tens                      ;Bazoume to plh8os twn dekadwn ston r24 
	ori r24,'0'                        ;Epeidh nwritera pollaplasiasame epipleon me to 10, oi "dekades" einai sthn pragmatikothta oi monades mas(akeraio meros)
	rcall usart_transmit                ;Ton metatrepoume se ascii kai stelnoume
	rcall lcd_data	

	ldi r24,','         
	rcall usart_transmit                ;Stelnoume to ','
	rcall lcd_data

	rcall get_units                     ;Bazoume to plh8os twn monadwn ston r24
	ori r24,'0'                        ;Epeidh nwritera pollaplasiasame epipleon me to 10, oi "monades" einai sthn pragmatikothta to klasmatiko meros
	rcall usart_transmit	            ;Ton metatrepoume se ascii kai stelnoume
	rcall lcd_data

	ldi r24,'\n'
	rcall usart_transmit                ;Stelnoume allagh grammhs
